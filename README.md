...
The report builder syncs reports to Supabase while retaining a local browser cache for fast/offline editing. No Supabase service-role key is required or supported in the frontend.

1. Open the Supabase SQL Editor and run [`supabase/migrations/001_create_wpcc_reports.sql`](supabase/migrations/001_create_wpcc_reports.sql).
2. Set `VITE_SUPABASE_URL` to your project URL.
3. Set either `VITE_SUPABASE_PUBLISHABLE_KEY` or `VITE_SUPABASE_ANON_KEY`.
4. Restart Vite after changing `.env.local`.

The migration creates `public.wpcc_reports`, indexes, an update timestamp trigger, and anonymous CRUD policies required by this no-login application.
